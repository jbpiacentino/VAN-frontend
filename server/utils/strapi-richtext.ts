...
export function withAttributes(value: unknown): Record<string, any> {
...
function blockNodeToMd(node: any): string {
  ...
  if (type === 'doc') return children.map((item: any) => blockNodeToMd(item)).filter(Boolean).join('\n\n');
  if (type === 'heading') {
...
export function pickStaticPageTitle(entry: Record<string, any>, fallback = 'Alliance Network') {
...
export function pickStaticPageContent(entry: Record<string, any>) {
